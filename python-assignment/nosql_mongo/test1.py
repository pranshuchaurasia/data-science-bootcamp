...
data = {
    "name": "kriti",
    "email": "kritin@gmail.com",
    "surname": "chaurasia",
    "subject": ["computer science", "statistics", "mathematics"]
}
database = client['myinfo']
collection2 = database["book_records"]
multiple_records = [
    {
        "isbn": "9781593279509",
...
# collection2.insert_many (multiple_records)

# record=collection2.find({"isbn":"9781491943533"})
# record = collection2.find({"isbn":{"$gt":"9781449365035"}})

# Find data in mongodb
# d= collection2.find({'status':'A'})
# d= collection2.find({'status':{'$in':['A', 'P']}})
# d= collection2.find({'qty':{'gte':75}})
# d= collection2.find({'item':'sketch_pad', 'qty': 95})
# d= collection2.find({'item':'sketch_pad', 'qty': {'gte':75}})
# d= collection2.find({ '$or': [ {'item':'sketch_pad'}, {'qty': {'gte':75}}]})

#collection2.update_one({'title': 'Pro Git'}, {'$set': {'title': 'Updating git is important'}})
collection2.delete_one({'title': 'Updating git is important'})
